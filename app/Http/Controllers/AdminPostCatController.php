...
class AdminPostCatController extends Controller
{
    ...
    function list(Request $requests) {
        ...
        if ($status == "active") {
            $post_cats = PostCat::withoutTrashed()->where("post_cat_title", "LIKE", "%{$key_word}%")->Paginate(20);
        } else if ($status == "licensed") {
            $list_act = [
                "pending" => "Chờ xét duyệt",
                "delete" => "Xoá tạm thời",
            ];
            $post_cats = PostCat::withoutTrashed()->where("post_cat_status", "licensed")->where("post_cat_title", "LIKE", "%{$key_word}%")->Paginate(20);
        } else if ($status == "pending") {
            $list_act = [
                "licensed" => "Đã đăng",
                "delete" => "Xoá tạm thời",
            ];
            $post_cats = PostCat::withoutTrashed()->where("post_cat_status", "pending")->where("post_cat_title", "LIKE", "%{$key_word}%")->Paginate(20);
        } else if ($status == "trashed") {
            $list_act = [
                "restore" => "Khôi phục",
                "delete_permanently" => "Xoá vĩnh viễn",
            ];
            $post_cats = PostCat::onlyTrashed()->where("post_cat_title", "LIKE", "%{$key_word}%")->Paginate(20);
        }
