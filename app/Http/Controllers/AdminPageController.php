...
class AdminPageController extends Controller
{
    ...
    function list(Request $requests) {
        ...
        if ($status == "active") {
            $pages = Page::withoutTrashed()->where("page_title", "LIKE", "%{$key_word}%")->Paginate(20);
        } else if ($status == "licensed") {
            $list_act = [
                "pending" => "Chờ xét duyệt",
                "delete" => "Xoá tạm thời",
            ];
            $pages = Page::withoutTrashed()->where("page_status", "licensed")->where("page_title", "LIKE", "%{$key_word}%")->Paginate(20);
        } else if ($status == "pending") {
            $list_act = [
                "licensed" => "Đã đăng",
                "delete" => "Xoá tạm thời",
            ];
            $pages = Page::withoutTrashed()->where("page_status", "pending")->where("page_title", "LIKE", "%{$key_word}%")->Paginate(20);
        } else if ($status == "trashed") {
            $list_act = [
                "restore" => "Khôi phục",
                "delete_permanently" => "Xoá vĩnh viễn",
            ];
            $pages = Page::onlyTrashed()->where("page_title", "LIKE", "%{$key_word}%")->Paginate(20);
        }
