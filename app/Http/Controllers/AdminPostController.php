...
class AdminPostController extends Controller
{
    ...
    function list(Request $requests) {
        ...
        if ($status == "active") {
            $posts = Post::withoutTrashed()->where("post_title", "LIKE", "%{$key_word}%")->Paginate(20);
        } else if ($status == "licensed") {
            $list_act = [
                "pending" => "Chờ xét duyệt",
                "delete" => "Xoá tạm thời",
            ];
            $posts = Post::withoutTrashed()->where("post_status", "licensed")->where("post_title", "LIKE", "%{$key_word}%")->Paginate(20);
        } else if ($status == "pending") {
            $list_act = [
                "licensed" => "Đã đăng",
                "delete" => "Xoá tạm thời",
            ];
            $posts = Post::withoutTrashed()->where("post_status", "pending")->where("post_title", "LIKE", "%{$key_word}%")->Paginate(20);
        } else if ($status == "trashed") {
            $list_act = [
                "restore" => "Khôi phục",
                "delete_permanently" => "Xoá vĩnh viễn",
            ];
            $posts = Post::onlyTrashed()->where("post_title", "LIKE", "%{$key_word}%")->Paginate(20);
        }
