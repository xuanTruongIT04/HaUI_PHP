...
class AdminBannerController extends Controller
{
    ...
    function list(Request $requests) {
        ...
        if ($status == "active") {
            $banners = Banner::withoutTrashed()->where("banner_name", "LIKE", "%{$key_word}%")->Paginate(20);
        } else if ($status == "licensed") {
            $list_act = [
                "pending" => "Chờ xét duyệt",
                "delete" => "Xoá tạm thời",
            ];
            $banners = Banner::withoutTrashed()->where("banner_status", "licensed")->where("banner_name", "LIKE", "%{$key_word}%")->Paginate(20);
        } else if ($status == "pending") {
            $list_act = [
                "licensed" => "Đã đăng",
                "delete" => "Xoá tạm thời",
            ];
            $banners = Banner::withoutTrashed()->where("banner_status", "pending")->where("banner_name", "LIKE", "%{$key_word}%")->Paginate(20);
        } else if ($status == "trashed") {
            $list_act = [
                "restore" => "Khôi phục",
                "delete_permanently" => "Xoá vĩnh viễn",
            ];
            $banners = Banner::onlyTrashed()->where("banner_name", "LIKE", "%{$key_word}%")->Paginate(20);
        }
