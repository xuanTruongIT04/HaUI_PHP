...
class AdminSliderController extends Controller
{
    ...
    function list(Request $requests) {
        ...
        if ($status == "active") {
            $sliders = Slider::withoutTrashed()->where("slider_name", "LIKE", "%{$key_word}%")->Paginate(20);
        } else if ($status == "licensed") {
            $list_act = [
                "pending" => "Chờ xét duyệt",
                "delete" => "Xoá tạm thời",
            ];
            $sliders = Slider::withoutTrashed()->where("slider_status", "licensed")->where("slider_name", "LIKE", "%{$key_word}%")->Paginate(20);
        } else if ($status == "pending") {
            $list_act = [
                "licensed" => "Đã đăng",
                "delete" => "Xoá tạm thời",
            ];
            $sliders = Slider::withoutTrashed()->where("slider_status", "pending")->where("slider_name", "LIKE", "%{$key_word}%")->Paginate(20);
        } else if ($status == "trashed") {
            $list_act = [
                "restore" => "Khôi phục",
                "delete_permanently" => "Xoá vĩnh viễn",
            ];
            $sliders = Slider::onlyTrashed()->where("slider_name", "LIKE", "%{$key_word}%")->Paginate(20);
        }
