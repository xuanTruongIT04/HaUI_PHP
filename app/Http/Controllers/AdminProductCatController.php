...
class AdminProductCatController extends Controller
{
    ...
    function list(Request $requests) {
        ...
        if ($status == "active") {
            $product_cats = ProductCat::withoutTrashed()->where("product_cat_title", "LIKE", "%{$key_word}%")->Paginate(20);
        } else if ($status == "licensed") {
            $list_act = [
                "pending" => "Chờ xét duyệt",
                "delete" => "Xoá tạm thời",
            ];
            $product_cats = ProductCat::withoutTrashed()->where("product_cat_status", "licensed")->where("product_cat_title", "LIKE", "%{$key_word}%")->Paginate(20);
        } else if ($status == "pending") {
            $list_act = [
                "licensed" => "Đã đăng",
                "delete" => "Xoá tạm thời",
            ];
            $product_cats = ProductCat::withoutTrashed()->where("product_cat_status", "pending")->where("product_cat_title", "LIKE", "%{$key_word}%")->Paginate(20);
        } else if ($status == "trashed") {
            $list_act = [
                "restore" => "Khôi phục",
                "delete_permanently" => "Xoá vĩnh viễn",
            ];
            $product_cats = ProductCat::onlyTrashed()->where("product_cat_title", "LIKE", "%{$key_word}%")->Paginate(20);
        }
