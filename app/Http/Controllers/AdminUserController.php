...
class AdminUserController extends Controller
{
    ...
    function list(Request $requests)
    {
        ...
        if ($status == "active") {
            $users = User::withoutTrashed()->where("fullname", "LIKE", "%{$key_word}%")->Paginate(20);
        } else if ($status == "licensed") {
            $list_act = [
                "pending" => "Chờ xét duyệt",
                "delete" => "Xoá tạm thời"
            ];
            $users = User::withoutTrashed()->where("user_status", "licensed")->where("fullname", "LIKE", "%{$key_word}%")->Paginate(20);
        } else if ($status == "pending") {
            $list_act = [
                "licensed" => "Đã cấp quyền",
                "delete" => "Xoá tạm thời"
            ];
            $users = User::withoutTrashed()->where("user_status", "pending")->where("fullname", "LIKE", "%{$key_word}%")->Paginate(20);
        } else if ($status == "trashed") {
            $list_act = [
                "restore" => "Khôi phục",
                "delete_permanently" => "Xoá vĩnh viễn"
            ];
            $users = User::onlyTrashed()->where("fullname", "LIKE", "%{$key_word}%")->Paginate(20);
        }
