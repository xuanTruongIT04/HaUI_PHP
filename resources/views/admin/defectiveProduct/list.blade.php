        ...
        <div class="card">
            ...
            <div class="card-header font-weight-bold d-flex justify-content-between align-items-center">
                <div id="title-btn-add">
                    <h5 class="m-0 ">Danh sách sản phẩm lỗi</h5>
                </div>
            ...
            <div class="card-body">
                ...
                <form action="{{ url('admin/defectiveProduct/action') }}" method="GET">
                    ...
                    @if (!empty(request()->key_word))
                        <div class="count-user"><span>Kết quả tìm kiếm: <b>{{ $count_defectiveProduct }}</b> sản phẩm lỗi</span></div>
                    @endif
                    <table class="table table-striped table-checkall">
                        ...
                        <tbody>
                                        ...
                                        @if (request()->status != 'trashed')
                                            <td>
                                                ...
                                                {{-- <a href="{{ route('admin.defectiveProduct.delete', $defectiveProduct->id) }}"
                                                    ...
                                                    data-toggle="tooltip"
                                                    onclick="return confirm('Bạn có chắc chắn muốn xoá sản phẩm lỗi {{ $defectiveProduct->product_name }}?')"
                                        ...
                                        @else
                                            <td>
                                                <a href="{{ route('admin.defectiveProduct.restore', $defectiveProduct->id) }}"
                                                    class="btn btn-success btn-sm rounded-0 text-white" type="button"
                                                    data-toggle="tooltip"
                                                    onclick="return confirm('Bạn có chắc chắn muốn khôi phục sản phẩm lỗi {{ $defectiveProduct->product_name }}?')"
                                                    data-placement="top" title="Restore"><i
                                                        class="fas fa-trash-restore-alt"></i></a>
                                                {{-- <a href="{{ route('admin.defectiveProduct.delete', $defectiveProduct->id) }}"
                                                    class="btn btn-danger btn-sm rounded-0 text-white" type="button"
                                                    data-toggle="tooltip"
                                                    onclick="return confirm('Bạn có chắc chắn muốn xoá vĩnh viễn sản phẩm lỗi {{ $defectiveProduct->product_name }}?')"
                                                    data-placement="top" title="Delete"><i class="fa fa-trash"></i></a> --}}
                                            </td>
                                        @endif
                                    </tr>
                            ...
                            @else
                                <tr>
                                    <td colspan="7" class="bg-white">Không tìm thấy sản phẩm lỗi nào!</td>
                                </tr>
                            @endif
                        </tbody>
                    </table>
