            ...
            <div class="card-body">
                <div class="analytic">
                    <a href="{{ request()->fullUrlWithQuery(['status' => 'active']) }}" class="text-primary">Kích
                        hoạt<span class="text-muted">({{ $count_productionPlan_status[0] }})</span></a>
                    <a href="{{ request()->fullUrlWithQuery(['status' => 'trashed']) }}" class="text-primary">Vô hiệu
                        hoá<span class="text-muted">({{ $count_productionPlan_status[1] }})</span></a>
                </div>
                                ...
                                @foreach ($productionPlans as $productionPlan)
                                    ...
                                    <tr>
                                        ...
                                        <td><a class="text-primary"
                                                href="{{ route('admin.productionPlan.edit', $productionPlan->id) }}">{{ $productionPlan->name_productionPlan }}</a>
                                        </td>
                                        ...
                                        @if (request()->status != 'trashed')
                                            <td>
                                                ...
                                                <a href="{{ route('admin.productionPlan.delete', $productionPlan->id) }}"
                                                    ...
                                                    data-toggle="tooltip"
                                                    onclick="return confirm('Bạn có chắc chắn muốn xoá kế hoạch sản xuất {{ $productionPlan->name_productionPlan }}?')"
                                        ...
                                        @else
                                            <td>
                                                <a href="{{ route('admin.productionPlan.restore', $productionPlan->id) }}"
                                                    class="btn btn-success btn-sm rounded-0 text-white" type="button"
                                                    data-toggle="tooltip"
                                                    onclick="return confirm('Bạn có chắc chắn muốn khôi phục kế hoạch sản xuất {{ $productionPlan->name_productionPlan }}?')"
                                                    data-placement="top" title="Restore"><i
                                                        class="fas fa-trash-restore-alt"></i></a>
                                                <a href="{{ route('admin.productionPlan.delete', $productionPlan->id) }}"
                                                    class="btn btn-danger btn-sm rounded-0 text-white" type="button"
                                                    data-toggle="tooltip"
                                                    onclick="return confirm('Bạn có chắc chắn muốn xoá vĩnh viễn kế hoạch sản xuất {{ $productionPlan->name_productionPlan }}?')"
                                                    data-placement="top" title="Delete"><i class="fa fa-trash"></i></a>
                                            </td>
                                        @endif
                                    </tr>
